    ...
    <div class="w-80 bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
        ...
        <div class="p-4 border-b border-gray-200">
            <div class="relative">
                ...
                <input wire:model.live.debounce.300ms="chatSearch" type="text" placeholder="Search clients..."
                       class="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1e3a8a] focus:border-[#1e3a8a]">
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            <button wire:click="selectChat({{ $chatItem->id }})" wire:key="chat-{{ $chatItem->id }}"
                    class="w-full flex items-center px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-100 text-left
                           {{ $chat && $chat->id === $chatItem->id ? 'bg-blue-50 border-l-4 border-l-[#1e3a8a]' : '' }}">
                ...
                <div class="ml-3 flex-1 min-w-0">
                    <div class="flex items-center justify-between">
                        ...
                        @if($chatItem->latestMessage)
                            <span class="text-xs text-gray-400 flex-shrink-0 ml-2">{{ $chatItem->latestMessage->created_at->shortRelativeDiffForHumans() }}</span>
                        @endif
                    </div>
                    <div class="flex items-center justify-between">
                        <p class="text-xs text-gray-500 truncate">
                            {{ $chatItem->latestMessage ? Str::limit($chatItem->latestMessage->body, 35) : 'No messages' }}
                        </p>
                        @if($chatItem->unread_messages_count > 0)
                            <span class="ml-2 inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-[#1e3a8a] rounded-full flex-shrink-0">
        ...
        @if($chat)
            ...
            <div class="px-6 py-3 bg-white border-b border-gray-200 flex items-center justify-between shadow-sm">
                ...
                <a href="{{ route('clients.show', $chat->client) }}"
                   class="text-xs text-[#1e3a8a] hover:underline font-medium">View Profile</a>
            </div>
                ...
                @foreach($messages as $message)
                    ...
                    @elseif($message->sender_id === auth()->id())
                        ...
                        <div class="flex justify-end">
                            <div class="max-w-sm">
                                <div class="bg-[#1e3a8a] text-white px-4 py-2.5 rounded-2xl rounded-br-md shadow-sm">
                    ...
                    @else
                        ...
                        <div class="flex justify-start">
                            <div class="max-w-sm">
                                <p class="text-xs text-gray-500 mb-1 ml-1">{{ $message->sender->name ?? 'Unknown' }}</p>
                                <div class="bg-white px-4 py-2.5 rounded-2xl rounded-bl-md shadow-sm ring-1 ring-gray-200">
            ...
            <div class="px-6 py-4 bg-white border-t border-gray-200">
                <form wire:submit="sendMessage" class="flex items-center space-x-3">
                    <input wire:model="messageBody" type="text" placeholder="Type a message..."
        ...
        @else
            ...
            <div class="flex-1 flex items-center justify-center">
                <div class="text-center">
                    ...
                    <h3 class="text-lg font-semibold text-gray-400">Select a chat</h3>
                    <p class="text-sm text-gray-400 mt-1">Choose a client conversation from the left panel.</p>
                </div>
            </div>
        @endif
    </div>
