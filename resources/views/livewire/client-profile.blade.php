    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    @if(!$editing)
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between items-center text-sm">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $colors[$client->status] ?? '' }}">{{ ucfirst($client->status) }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-500">Assigned To</span>
                                <span class="font-medium text-gray-900">{{ $client->assignedUser->name ?? 'Unassigned' }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-500">Last Update</span>
                                <span class="font-medium text-gray-900">{{ $client->last_update_at ? $client->last_update_at->diffForHumans() : 'Never' }}</span>
                            </div>
                        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Add Follow-up Update</h3>
                <form wire:submit="addUpdate" class="space-y-4">
                    <div>
                        <input wire:model="updateTitle" type="text" placeholder="Update title (e.g., 'Called client', 'Sent proposal')"
                    ...
                    <div>
                        <textarea wire:model="updateNotes" rows="3" placeholder="Additional notes..."
            ...
            {{-- Update History --}}
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Update History</h3>
                <div class="space-y-4">
                    @forelse($updates as $update)
                    <div class="relative pl-8 pb-4 {{ !$loop->last ? 'border-l-2 border-gray-200' : '' }} ml-3">
                        <div class="absolute -left-[9px] top-0 w-5 h-5 bg-[#1e3a8a] rounded-full flex items-center justify-center">
                    ...
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-400 text-sm">Add the first follow-up update above.</p>
                    </div>
                    @endforelse
                </div>
            </div>
